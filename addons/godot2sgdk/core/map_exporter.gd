...
func export_tilemaps_in_scene(scene_root: Node, export_path: String) -> Dictionary:
	...
	print("📊 [MapExporter] Found %d tilemaps" % tilemaps.size())
	...
	for tilemap in tilemaps:
		...
		var collision_data = export_tilemap_collision(tilemap)  # ✅ NOVO: Exportar colisão

		result[tilemap.name] = {
			"tilemap_data": tilemap_data,
			"collision_data": collision_data  # ✅ Adicionar dados de colisão
		}
		
		# Gerar headers
		_generate_tilemap_header(tilemap, tilemap_data)
		_generate_tilemap_collision_header(tilemap, collision_data)  # ✅ NOVO: Header de colisão
	...
	if header_file:
		...
		header_file.close()
		result.success = true
		result.message = "Exportação concluída: %d tilemaps exportados" % tilemaps.size()
	else:
...
func _generate_tilemap_collision_header(tilemap: TileMap, collision_data: Dictionary) -> String:
	...
	# Exportar formas de colisão (implementação simplificada)
...
func _find_tilemaps_in_node(node: Node) -> Array:
	...
	if node is TileMap or node is TileMapLayer:
		...
		print("📍 Found TileMap: ", node.name, " (Type: ", node.get_class(), ")")
...
func _process_tilemap(tilemap_node: Node) -> Dictionary:
	...
	print("🛠️ Processing tilemap: ", tilemap_node.name, " (Layers: ", result["layers"].size(), ")")
	return result
...
func _process_tilemap_layer(tilemap_node: Node, layer_idx: int) -> Dictionary:
	...
	for cell in used_cells:
		...
		if tile_data:
			# ✅ CORREÇÃO PARA Godot 4.4.1 - usar métodos disponíveis
